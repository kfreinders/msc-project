...
compute_summary_statistics <- function(SimulationSingle, nosoi_settings) {
  ...
  SS_19 <- transitivity(graph, type = "global") # Clustering coefficient
  SS_20 <- edge_density(graph)                  # Network density
  SS_21 <- diameter(graph)                      # Network diameter
  SS_22 <- safe_mean(ego_size(graph))           # Mean neighborhood size
  SS_23 <- radius(graph)                        # Network radius
  SS_24 <- safe_mean(alpha_centrality(graph))   # Network mean alpha centrality
  SS_25 <- global_efficiency(graph)             # Network global efficiency
